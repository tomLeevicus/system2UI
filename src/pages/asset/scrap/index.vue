...
const handleQuery = async () => {
  ...
  const [err, res] = await getScrapList({
    ...queryParams,
    pageNum: current,
    pageSize: size,
  });
...
const handleApproveSubmit = async () => {
  ...
  try {
    const [error, res] = await approveScrap({
      scrapRecordId: approveDialog.form.id!,
      approvalComment: approveDialog.form.remark,
      isAgreed: approveDialog.form.status,
    });
...
const columns = [
  ...
  },
  {
    ...
    render: (scope: any) => (
      <>
        <el-button type="primary" link onClick={() => handleApprove(scope.row)}>
          审核
        </el-button>
      </>
    ),
